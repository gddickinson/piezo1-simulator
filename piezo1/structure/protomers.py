"""Which chains are protomers, and which residues all three of them share.

Both questions are load-bearing. A block set built from chains that do not share
a residue basis produces an elastic network of the wrong size, and every result
downstream is then a plausible wrong number rather than an error.

This lived in ``ui/model_utils.py`` until :mod:`piezo1.structure.frame` needed
it. That import would have pointed ``structure → ui``, against the project's
dependency arrow, and ``analysis/claims.py`` was already reaching into ``ui``
for the same function — which quietly made a headless analysis depend on PyQt.
The helpers are structural operations, so they belong here; ``ui.model_utils``
