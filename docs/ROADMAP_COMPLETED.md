...
across 75 rounds, plus the five-round review sections that decided what came
next.

This is the *result* record. `SESSION_LOG.md` is the *reasoning* record — why a
round was done the way it was, and what went wrong on the way. Where they
overlap, the session log is the fuller account; this file is the index of
outcomes.

`ROADMAP.md` holds what is still open.

**Nothing here may be quietly dropped.** `tests/test_roadmap.py` checks every
completed item against the pre-split file in git, and ratchets the count so it
cannot fall.
...
- [x] **The original Block O entry, ticked at last.** It asked: may a
      conductance or selectivity change stand for gain or loss of
      mechanosensitive function? Answered in `analysis/engineered.py` — **no**,
      on the project's own annotations. A2078W loses Yoda1 sensitivity while
      retaining stretch response, and KKKK2166- removes inactivation with
      mechanics unchanged, so the two axes dissociate. Five of the fifteen are
      admissible in principle; admitting them adds **0** discriminating
      positions, the one engineered pair (1335) being same-direction.
- [x] *Validate:* satisfied. `variant_sets` still refuses to pool evidence
      levels — the five were never admitted — and `tests/test_variant_sets.py`
      and `tests/test_engineered.py` both pass.
- [x] **Round 75 found this entry still unticked**, under a second `Round 68`
      heading directly below the supersession note above. Round 67 recorded the
      supersession by *adding* a heading rather than ticking the original, so
      the file carried the same round number twice — once done, once open — and
      three hundred ticked boxes hid it. Merged here; the duplicate heading is
      gone and nothing it said was dropped.
...
- [x] **Done.** 2,702 lines, 96% of it a record of finished work that
      duplicated `SESSION_LOG.md`. Split into this archive and a 124-line
      `ROADMAP.md` holding only what is open. The rule was mechanical — a
      section goes wholesale to one file or the other — and every section
      turned out to be entirely open or entirely done, so nothing had to be
      cut in half.
- [x] *Validate:* **no completed item was lost, and that is checked rather than
      asserted.** `tests/test_roadmap.py` reads the pre-split file out of git at
      commit `4c1c61c` and compares item by item: 358 completed items before,
      all 358 present after, none in both files, every open item in
      `ROADMAP.md` and none in the archive. Frozen counts back it up where git
      history is unavailable, and the completed count is ratcheted.
- [x] **What the length was hiding.** Two adjacent headings both numbered
      Round 68 — one ticked as superseded by Round 63, one still open asking the
      identical question — because Round 67 recorded the supersession by adding
      a heading rather than ticking the original. Merged, with the duplication
      itself written down. This is the second time a roadmap entry has been
