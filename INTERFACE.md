...
| `result_dialog.py` | A readable window for an analysis that returns a table rather than a picture. Generic over the shared `ANALYSES` result, so it cannot fall out of step with the functions it displays. | `ResultDialog`, `format_result()` | ✅ |
