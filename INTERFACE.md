...
| `feasibility.py` | **Whether the central claim is testable at all.** Reads the recorded effect sizes and asks what sample size they need: the substitution-aware predictor's δ = −0.249 needs 134 variants against an optimistic ceiling of 59, where power is 0.51. Runs no comparison — a test enforces that. | `assess()`, `FeasibilityReport`, `Scenario`, `recorded_round()`, `observed_effect()`, `modelling_survival()`, `observed_split()` | ✅ |
...
| `test_conclusion.py` | The guard on the one page a reader is meant to trust without checking: every number must come from the claims registry, the validation record or `HEADLINE`, with an allowlist whose entries each state a reason. Calibrated — an invented number must fail. Also pins that all five nulls appear, that the unprovability result is stated rather than just the failures, and that the page is reachable from the README and the help. | ✅ |
